...
| やねうら王 | 9.70git / `76d58ef` | Suisho11Plus の教師エンジン |
| Suisho11Plus | 2026-08-01 配布記事に対応する手元アーカイブ | SFNN_halfka2_1024_7_64_k3k3、FV_SCALE=40 |

Suisho11Plus は重みであり、やねうら王の実行ファイルとは別。手元の実行ファイル配布物は macOS 用だったため、公開履歴から配布記事と同じ V9.70 系のバージョン更新 commit を特定し、Linux 向けにビルドした。配布バイナリとのビット単位の同一性は主張しない。教師の変更時は新しい比較系列として扱う。
...
ノード上限到達が aspiration 探索の途中になると、やねうら王の最終 `info` に上限・下限が付くことがある。`OutputFailLHPV=false` でも最後の報告には付く場合がある。これはノード指定の疎通失敗ではないが、確定値の採点には使えない。正式グラフ比較での扱いは研究方針にある次の確認事項。smoke では境界の向きと生の値を保存し、先後反転では上限／下限も反転する。
...
正式棋譜の選定・分割、複数棋譜の比較、CPU での小規模学習・重み読込の検証は別の到達点であり、上記の利用範囲内で進める。初回 smoke の結果は [初期検証](validation/environment-2026-09-14.md) に記録する。
